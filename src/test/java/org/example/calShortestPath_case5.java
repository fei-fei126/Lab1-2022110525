...
class calcShortestPath_case5 {
    ...
    void calcShortestPath() {
        ...
        List<String> expectedPath = Arrays.asList("scientist", "analyzed", "the", "data");
        int expectedLength = 3; // 3 edges between 4 nodes
        ...
        assertTrue(output.contains("计算从 scientist 到 data 的最短路径"),
                "Should show calculation start message");
        assertTrue(output.contains("scientist 和 data 之间的路径:"),
                "Should list all found paths");
        assertTrue(output.contains("最短路径图已保存到"),
                "Should confirm image generation");
        assertTrue(output.contains("[scientist, analyzed, the, data]"),
