...
public class Code1 {
    ...
    private void selectFile(JTextField filePathField) {
        ...
        if (returnValue == JFileChooser.APPROVE_OPTION) {
            ...
            if (!dir.exists() && !dir.mkdirs()) { // 检查mkdirs的返回值
                outputArea.append("无法创建输出目录: " + dir.getAbsolutePath() + "\n");
                return; // 退出方法，因为无法创建目录
            }
    ...
    private void showBridgeWordsDialog() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            if (word1.isEmpty() || word2.isEmpty()) {
                outputArea.append("You must enter two words.\n");
                return;
            }

            // 检查是否只包含字母
            if (!word1.matches("[a-zA-Z]+") || !word2.matches("[a-zA-Z]+")) {
                outputArea.append("The input must consist of English alphabetic characters only.\n");
                return;
            }
    ...
    public void createDotFile(String dotFilePath, List<List<Object>> shortestPaths, String root) {
        try (PrintWriter writer = new PrintWriter(dotFilePath, StandardCharsets.UTF_8)) {
    ...
    public Set<String> queryBridgeWords(String start, String end, Boolean print) {
        ...
        if (!directedGraph.containsKey(start) && print) {
            outputArea.append("No " + start + " in the graph!\n");
            return null;
        }
        if (!directedGraph.containsKey(end) && print) {
            outputArea.append("No " + end + " in the graph!\n");
            return null;
        }
        ...
        for (Map.Entry<String, Integer> entry : directedGraph.getOrDefault(current, Collections.emptyMap()).entrySet()) {
            ...
            if (!visited.contains(neighbor)) {
                ...
                if (directedGraph.containsKey(neighbor) && directedGraph.get(neighbor).containsKey(end)) {
                    ...
                    if (print) {
                        outputArea.append("The bridge words from " + start + " to " + end + " are: " + neighbor + "\n");
                    }
                }
            }
        }
        ...
        if (bridgeWords.isEmpty() && print) {
            outputArea.append("No bridge words from " + start + " to " + end + "!\n");
        }
        return bridgeWords;
    }
    ...
    public String randomWalk() {
        // 创建一个随机数生成器。
        SecureRandom secureRandom = new SecureRandom();
        // 将图的所有节点（directedGraph.keySet()）存储到一个列表中。
        List<String> vertices = new ArrayList<>(directedGraph.keySet());
        // 从节点列表中随机选择一个节点作为起始节点。
        ...
        try (Writer writer = new FileWriter(outputPath, StandardCharsets.UTF_8);
            ...
            printWriter.println(randomWalkText.toString());
        } catch (IOException e) {
    ...
    public Map<String, Double> calculatePageRank(double dampingFactor, int iterations) {
        ...
        for (Map.Entry<String, Map<String, Integer>> entry : directedGraph.entrySet()) {
            pageRank.put(entry.getKey(), initialValue);
        }
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            for (Map.Entry<String, Map<String, Integer>> entry : directedGraph.entrySet()) {
                if (entry.getValue().isEmpty()) {
                    danglingPR += pageRank.get(entry.getKey());
                }
            }
            ...
            // 计算每个节点的新PR值
            for (Map.Entry<String, Map<String, Integer>> uEntry : directedGraph.entrySet()) {
                String u = uEntry.getKey();
                double sum = 0.0;

                // 找出所有指向u的节点
                for (Map.Entry<String, Map<String, Integer>> vEntry : directedGraph.entrySet()) {
                    if (vEntry.getValue().containsKey(u)) {
                        int L_v = vEntry.getValue().size();
                        sum += pageRank.get(vEntry.getKey()) / L_v;
                    }
                }
        ...
        if (result == JOptionPane.OK_OPTION) {
            try {
                ...
                for (Map.Entry<String, Double> entry : sortedPR) {
                    outputArea.append(String.format("%s: %.6f%n", entry.getKey(), entry.getValue()));
                }
    ...
    private void savePageRankResults(Map<String, Double> pageRank, String filePath) {
        ...
        try (Writer writer = new FileWriter(filePath, StandardCharsets.UTF_8);
            ...
            printWriter.println("PageRank结果(从高到低):");
            for (Map.Entry<String, Double> entry : sorted) {
                printWriter.printf("%s: %.6f%n", entry.getKey(), entry.getValue());
            }
        } catch (IOException e) {
            // 处理异常
    ...
    // 假设存在一个输出区域用于显示错误信息
    private void outputArea(String message) {
